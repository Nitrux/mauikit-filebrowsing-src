    ...
    return FMH::MODEL {
        ...
        const FMH::MODEL FMStatic::getFileInfoModel(const QUrl &path)
        {
            ...
            }else
            {
                ...
                {FMH::MODEL_KEY::DATE, file.birthTime(QTimeZone::UTC).toString(Qt::TextDate)},
                {FMH::MODEL_KEY::MODIFIED, file.lastModified(QTimeZone::UTC).toString(Qt::TextDate)},
                {FMH::MODEL_KEY::LAST_READ, file.lastRead(QTimeZone::UTC).toString(Qt::TextDate)},
